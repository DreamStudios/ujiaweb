...
//	})
	...
	//包养
	$('.joke_kept').live('click',function(){
		islogin();
		if(!loginState){return false;};
		var htm,kept_num;
		kept_num=$(this).attr('data-id');
		htm='<form id="form_buy_box">';
		htm+='<div style="width:0;height:0;overflow:hidden">';
		htm+='<input type="text" name="joke_id" id="joke_id" value="" />';
		htm+='</div>'
		htm+='<p class="title">包养我吧！只需<span>'+kept_num+'</span>麻币哦！</p>';
		htm+='<p class="title">以后再赚钱就是你的了</p>';
		htm+='<p class="buy_error_message"></p>';
		htm+='<p class="input-btn"><input type="submit" value="确定" class="reward-submit" /><input type="button" class="reward-close" value="取消" /></p>';
		htm+='</form>';
		htm+='<div class="buy_box_message">';
		htm+='</div>';
		
		
		var tthis = $(this),
			id = tthis.parent().parent('dd').parent('dl').attr('mahua'),
			x = tthis.offset().left-95,
			y = tthis.offset().top-295;
		if($('#reward').length<=0){$('body').append('<div id="reward" class="joke-buy-box"></div>')}
		if($('#reward-bg').length<=0){$('body').append('<div id="reward-bg"></div>')}
		$('#reward').removeClass('obtain-box').addClass('buy-box');
		$('#reward').html(htm);
		$('#joke_id').val(id);
		$('#reward-bg').css('display','block');
		if ($.browser.msie && ($.browser.version == "6.0") && !$.support.style){
			$('#reward-bg').css('height',$(window).height()).css('top',$(window).scrollTop());
		}
		$('#reward').css('display','block').css('top',y+'px').css('left',x+'px');
		
		$('.reward-close , #reward-bg').click(function(){
			$('#reward').css('display','none').html('');
			$('#reward-bg').css('display','none');
		})

		$.validator.setDefaults({       
			submitHandler: function() {
				$('.reward-submit').attr("disabled",true).val('提交中..').css('background','#bcbcbd');
				$.ajax({
					url : keptUrl+'?callback=?',
					dataType : 'jsonp',
					data : $('#form_buy_box').serialize(),
					success : function(ret){
						if(ret.code > 0) {
							$('#reward').addClass('buy-box-success');
							$('#form_buy_box').css('display','none');
							$('.buy_box_message').css('display','block').html(ret.message);
							setTimeout(function(){
								$('#reward').css('display','none').html('');
								$('#reward-bg').css('display','none');
								tthis.parent('.buy').addClass('kepted').html(ret.data.html);
							},3000)
						} else {
							$('.buy_error_message').html(ret.message);
							$('.reward-submit').attr("disabled",false).val('确定').css('background','#ff845b');
						}
					}
				})
			}
		}),
		$('#form_buy_box').validate({
			onkeyup : false,
			rules:{
				joke_id : {
					required : true,
			...
			errorPlacement: function (error, element) { 
				if (element.is(':radio') || element.is(':checkbox')) {
					...
					$('.buy_error_message').text(error.text());
				}
			},
			focusInvalid: true
		})
	...
	//打赏
	$('.reward , .rewarded').live('click',function(){
		islogin();
		if(!loginState){return false;};
		var htm;
		var tthis=$(this),
			message_s=tthis.parent('.operation').attr('data-score'),
			id = tthis.parent('dd').parent('dl').attr('mahua'),
			x = tthis.offset().left-100,
			y = tthis.offset().top-300;
		if(typeof(message_s)=='undefined'){
			message_s=0;
		}
		htm='<form id="form_reward">';
		htm+='<dl>'
		htm+='<dt>大爷们赏了<span>'+message_s+'</span>麻币</dt>';
		if(tthis.hasClass('rewarded')){
			var m=tthis.attr('data-maryaneScore');
			htm+='<dd class="oed">';
			htm+='<p class="title">你给TA打赏了<span>'+m+'</span>麻币</p>';
			htm+='<p class="title">只能打赏一次哦 ^_^</p>';
			htm+='</dd>';
			htm+='<dd class="input-btn"><input type="button" class="reward-close" value="取消" /></dd>';
		}else{
			htm+='<dd class="input-radio">';
			htm+='<div style="width:0;height:0;overflow:hidden">';
			htm+='<input type="text" name="joke_id" id="joke_id" value="" />';
			htm+='<input type="text" name="maryane_level" id="maryane_level" value="2" />';
			htm+='</div>';
			htm+='<span data-id="1">5麻币</span>';
			htm+='<span data-id="2" class="hover">10麻币</span>';
			htm+='<span data-id="3">20麻币</span>';
			htm+='<span data-id="4">30麻币</span>';
			htm+='<span data-id="5">40麻币</span>';
			htm+='<span data-id="6">50麻币</span>';
			htm+='</dd>';
			htm+='<dd class="message"></dd>';
			htm+='<dd class="input-btn"><input type="submit" value="打赏" class="reward-submit" /><input type="button" class="reward-close" value="取消" /></dd>';
		}
		htm+='</dl>';
		htm+='</form>';
		...
		if($('#reward').length<=0){$('body').append('<div id="reward" class="joke-buy-box"></div>')}
		if($('#reward-bg').length<=0){$('body').append('<div id="reward-bg"></div>')}
		$('#reward').removeClass('buy-box').removeClass('buy-box-success').addClass('obtain-box');
		$('#reward').html(htm);
		$('#reward-bg').css('display','block');
		if ($.browser.msie && ($.browser.version == "6.0") && !$.support.style){
			$('#reward-bg').css('height',$(window).height()).css('top',$(window).scrollTop());
		...
		//打赏金额选择
		$('.input-radio').find('span').click(function(){
			$('.input-radio').find('span').removeClass('hover');
			$(this).addClass('hover');
			$('#maryane_level').val($(this).attr('data-id'));
		})
		$.validator.setDefaults({
			submitHandler: function() {
				$('.reward-submit').attr("disabled",true).val('打赏中..').css('background','#bcbcbd');
				$.ajax({
					url : maryaneUrl+'?callback=?',
					dataType : 'jsonp',
					...
					success : function(ret){
						if(ret.code > 0) {
							$('#reward').css('display','none').html('');
							$('#reward-bg').css('display','none');
							tthis.addClass('rewarded').removeClass('reward').html('已打赏').attr('data-maryaneScore',ret.data.maryaneScore);
							tthis.parent('.operation').attr('data-score',ret.data.score);
							$.tips({text:ret.message});
						} else {
							$('.message').html(ret.message);
							$('.reward-submit').attr("disabled",false).val('打赏').css('background','#ff845b');
						}
					}
				})
			}
		}),
		$('#form_reward').validate({
			onkeyup : false,
			rules:{
				joke_id : {
					required : true,
...
function joke_content_area(){
	$('dl.mahua').each(function() {
		...
		if(content_height>900){
			...
			tthis.after('<dd class="joke-content-area"><a href="javascript:void(0)" class="joke-height-k" data-height="'+content_height+'">展开</a></dd>');
		}
	});
	$('.joke-content-area').find('.joke-height-k').live('click',function(){
		...
		$(this).removeClass('joke-height-k').addClass('joke-height-h').text('收起');
	...
	$('.joke-content-area').find('.joke-height-h').live('click',function(){
		...
		$(this).removeClass('joke-height-h').addClass('joke-height-k').text('展开');
		$('.joke-content-area').css('background-position','-314px -322px').css('line-height','22px');
	})
}

//背景定位兼容代码
//backgroundPosition兼容IE6+、opera、chrome、firefox、safari的背景定
(function($) {
...
function submit_comment(){
	...
	if(content==''){
		$('#comment').find('.message').html('您还没有写评论的内容');
		return false;
	}else if(content.length>140){
		$('#comment').find('.message').html('最多只能写140字哦~');
		return false;
	}else if(content.length<2){
		$('#comment').find('.message').html('至少要写两个字吧');
		return false;
	}else{
		$('#comment-submit').attr("disabled",true).val('提交中...').css('background','#bcbcbd');
		$.ajax({
			...
			success : function(ret){
				if(ret.code>0){
					...
					$('#comment-submit').val('完成');
					c.val(c.attr('title'));
					$('#comment-list').prepend(ret.data.html);
					setTimeout(function(){
						$('#comment-submit').attr("disabled",false).val('评论').css('background','#04ce9b');
					},10000);
...
function jokeListSubmitComment(c){
	...
	if(content==''){
		tthis.find('.text-length').css('color','#ff4000').text('您还没有写评论的内容');
		return false;
	}else if(content.length>140){
		tthis.find('.text-length').css('color','#ff4000').text('最多只能写140字哦~');
		return false;
	}else if(content.length<2){
		tthis.find('.text-length').css('color','#ff4000').text('至少要写两个字吧');
		return false;
	}else{
		tthis.find('.comment-submit').attr("disabled",true).val('提交中...').css('background','#bcbcbd');
		$.ajax({
			url : addCommentUrl+'?callback=?',
			dataType : 'jsonp',
			data : {
		...
		success : function(rot){
			if(rot.code > 0) {
				$.each(rot.data,function(i,v){
					...
					tthis.find('.comment').find('i').text(v.comment.count+'条评论');
					tthis.find('.operation').attr('data-score',v.maryane.score);
					if(v.kept.status==1){
						tthis.find('.buy').html('<a href="javascript:void(0)" class="joke_kept" data-id="'+v.kept.score+'">包养</a>');
					...
					if(v.maryane.status==2){
						...
						tthis.find('.buy').before('<a class="rewarded" data-maryaneScore="'+maryaneScore+'" href="javascript:void(0)">已打赏</a>');
					}else if(v.maryane.status==1){
						tthis.find('.buy').before('<a class="reward" href="javascript:void(0)">打赏</a>');
					}
...
function islogin(lUrl){
	...
	if(parseInt($.cookie('MH_USER_ID'))>0){
		...
		htm+='<dt>快乐麻花 欢迎你回来！</dt>';
		htm+='<dd class="loginp"><span class="title">使用麻花帐号登录</span><span id="login-prompt"></span></dd>';
		htm+='<dd>';
		htm+='<form id="form_login">';
		htm+='<ul>';
		htm+='<li><input type="text" name="email" class="username" title="邮箱/手机号/用户名"/><span class="pass-value pass-value-login">邮箱/手机号/用户名</span></li>';
		htm+='<li><input type="password" name="password" class="password" title="密码"/><span class="pass-value pass-value-login">密码</span></li>';
		htm+='<li><div class="user-action">';
		htm+='<input type="checkbox" name="keep" id="keep" value="1" checked /><label class="Remember-account" for="keep">下次自动登录</label>';
		htm+='<a href="'+userDomain+'member/forgetpassword" target="_blank">忘记密码？</a><input type="hidden" id="token" name="token" value="" />';
		htm+='</div></li>';
		htm+='<li><input type="submit" class="btn" value="登录"/></li>';
		htm+='</ul>';
		htm+='</form>';
		htm+='</dd>';
		htm+='</dl>';
		htm+='<div class="login-right">';
		htm+='<div class="or"><span>或</span></div>';
		htm+='<dl class="loginp">';
		htm+='<dt class="title">使用以下帐号直接登录</dt>';
		htm+='<dd class="weibo"><a href="'+userDomain+'connect/weibo" target="_blank"><i class="login-weibo"></i><span>新浪微博</span></a></dd>';
		htm+='<dd class="qq"><a href="'+userDomain+'connect/tencent" target="_blank"><i class="login-qq"></i><span>QQ</span></a></dd>';
		htm+='<dd class="login-reg"><a href="'+passportDomain+'account/register" class="btn">10秒钟注册</a></dd>';
		htm+='</dl>';
		htm+='</div>';
		htm+='</div>';
		if($('#login').length<=0){
...
function listTopFocus(){
	var sWidth = $("#listTopFocus").width(); //获取焦点图的宽度（显示面积）
	var len = $("#listTopFocus ul li").length; //获取焦点图个数
	var index = 0;
	var picTimer;
	...
	function showPics(index) { //普通切换
		...
		$("#listTopFocus ul").stop(true,false).animate({"left":nowLeft},800); //通过animate()调整ul元素滚动到计算出的position
		$("#listTopFocus .btn .point").removeClass("on").eq(index).addClass("on"); //为当前的按钮切换到选中的效果
	}
...
function newbieGuide_b(){
	if($('.joke_kept').length>0){
		...
		$('body').append('<div id="newbieGuide2" data-newbie-id="2"><a href="javascript:void(0);" rel="nofollow" class="newbieGuideClose">关闭</a></div>')
...
function newbieGuide_c(){
	if($('.reward').length>0){
		...
		$('body').append('<div id="newbieGuide3" data-newbie-id="3"><a href="javascript:void(0);" rel="nofollow" class="newbieGuideClose">关闭</a></div>')
...
function newbieGuide_d(){
	if($('.sell-joke').length>0){
		...
		$('body').append('<div id="newbieGuide4" data-newbie-id="4"><a href="javascript:void(0);" rel="nofollow" class="newbieGuideClose">关闭</a></div>')
...
function regGuideLucky(){
	...
	htm+='	<p>本活动主要针对新用户，已有麻花账号的用户不再给予奖励</p>'
	htm+='	<div class="btn">'
	htm+='		<a href="'+ passportDomain +'account/register" target="_blank" class="reg-guide-lucky-login">注册领取</a>'
	htm+='		<a href="'+ userDomain +'connect/tencent" target="_blank" rel="nofollow" class="right-login-qq"><i></i><span>QQ登录</span></a>'
	htm+='		<a href="'+ userDomain +'connect/weibo" target="_blank" rel="nofollow" class="right-login-weibo"><i></i><span>微博登录</span></a>'
	htm+='	</div>'
	htm+='	<dl>'
	htm+='		<dt>麻币能干什么？</dt>'
	htm+='		<dd>'
	htm+='			麻币可用来换取快乐麻花所提供的精美礼品，如:iPhone 6、笔记本电脑、iPad、鼠标、键盘等。'
	htm+='		</dd>'
	htm+='	</dl>'
	htm+='</div>'
	htm+='</div>'
...
function regGuide(){
	...
	htm+='			<dt>'
	htm+='				<p>等了好久...</p>'
	htm+='				<p>您终于加入快乐麻花大家族啦！</p>'
	htm+='			</dt>'
	htm+='			<dd><b>·</b>原创笑话和趣图，认准快乐麻花！</dd>'
	htm+='			<dd><b>·</b>打赏、包养、评论、投稿、审稿，想怎么玩就怎么玩！</dd>'
			...
			$.ajax({
				...
				success : function(ret){
					if(ret.code > 0) {
						...
						$('.eidtQQNum-submit').val('保存成功').attr("disabled",true).css('color','#ccc').show();
					} else {
						alert(ret.message);
					}
			...
			$.ajax({
				...
				success : function(ret){
					if(ret.code > 0) {
						...
						$('.eidtMobilePhone-submit').val('保存成功').attr("disabled",true).css('color','#ccc').show();
					} else {
						alert(ret.message);
					}
